...
// Find element by year
var getElementByYear = (canIndex, chiIndex) => {
...
var getYearName = (year) => {
  ...
  const element = getElementByYear(thienCanIndex, diaChiIndex);
...
var getYearNameByAge = (age) => {
  const year = (new Date()).getFullYear() - age;
  return getYearName(year);
}

// test
for (let i = 1940; i <= 2050; i++) {
  console.log(`Tuổi âm lịch 12 con giáp của năm ${i} là: ${getYearName(i)}`);
}

// test
for (let i = 20; i < 90; i++) {
  console.log(`Tuổi âm lịch 12 con giáp ở tuổi ${i} là: ${getYearNameByAge(i)}`);
}
